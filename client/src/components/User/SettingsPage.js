...
import { getRandomPost } from "../../actions/PostActions";
...
const SettingsPage = () => {
    ...
    const LoadUserData = async () => { 
        const user_Data = await getUserWithId(cookies.get('user_data')["_id"])
                ...
                { user["image"]?.split("")?.length === 0 &&
                    <div class="flex items-center justify-center w-[95%] mt-2">
                        <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer  hover:bg-bray-800 bg-gray-700  border-gray-600 hover:border-gray-500 hover:bg-gray-600">
                            ...
                            <input onChange={(e) => handleImageUpload(e)} accept="image/*" id="dropzone-file" type="file" class="hidden" />
                        </label>
                    </div> 
                }           
            <h1 className=" font-Title text-4xl mt-0"> Profile Image </h1>
            <div className=" flex w-full items-center justify-center gap-4">
                {user["image"] !== "" &&
                    <button onClick={() => remove_image()} className=" btn btn-sm btn-error text-white hover:opacity-80"> Change </button>
                }

                <button onClick={() => GetRandomProfileImage()} className=" btn btn-sm btn-primary">Random</button>
            </div>
                

            <div className="form-control w-[90%] mb-8">
                ...
                <textarea 
                    ...
                    maxLength={250}
                    className="textarea textarea-bordered h-24 outline-none mb-2" 
                    value={user.bio}
                    placeholder="I find and share cool images and collections of car posts.">
                </textarea>
                <button onClick={() => UpdateLoadUserData()} className=" btn btn-sm btn-primary"> Update </button>
            </div>

            
            <button onClick={() => navigate("/Profile/"+user["_id"])} className=" btn btn-sm btn-primary mb-16"> View Profile </button>
